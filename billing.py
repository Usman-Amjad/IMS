...
class BillClass:
    def __init__(self, root):
        ...
        lbl_note = Label(ProductFrame1, text="Note: Enter 0 Quantity to remove the product from cart", font=("goudy old style", 12), anchor="w", bg="white", fg="red").pack(side=BOTTOM, fill=X)
        ...
        btn_clear_cart = Button(Add_CartWidgetsFrame, text="Clear", command=self.clear_cart, font=("imes new roman", 15, "bold"), bg="lightgray", cursor="hand2").place(x=180, y=70, width=150, height=30)
        btn_add_cart = Button(Add_CartWidgetsFrame, command=self.add_update_cart, text="Add | Update Cart", font=("imes new roman", 15, "bold"), bg="orange", cursor="hand2").place(x=340, y=70, width=180, height=30)
        ...
        btn_generate = Button(billMenuFrame, command=self.generate_bill, text="Generate\nSave Bill", cursor="hand2", font=("goudy old style", 15, "bold"),
                              bg="#009688", fg="white")
        btn_generate.place(x=246, y=80, width=160, height=50)

        # ===== Footer =====
        footer = Label(self.root, text="IMS-Inventory Management System | Developed By UA\nFor any Technical Issue contact: +923448112288", font=("times new roman",11), bg="#4d636d", fg="white").pack(side=BOTTOM, fill=X)
...
# ==================== All Functions ==========================================
    ...
    def bill_top(self):
        ...
        bill_top_temp = f'''
\t\t XYZ-Inventory
\t Phone No. +923213224114, Nawabshah
{str("="*47)}
...
{str("="*47)}\n
    ...
    def print_bill(self):
        if self.chk_print == 1:
            ...
            messagebox.showinfo('Print', "Pleasse generate bill, to print the receipt", parent=self.root)
